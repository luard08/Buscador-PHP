<?php
//eer el archivo que contiene la informacion
function readData(){
  ...
  if($filtroCiudad == "" and $filtroTipo=="" and $filtroPrecio==""){ 
    ...
    $mayor = $filtroPrecio[1]; 
      ...
      if($filtroCiudad != "" and $filtroTipo == ""){ 
          foreach ($data as $index => $item) {
            $precio = precioNumero($item['Precio']);
            if ($filtroCiudad == $item['Ciudad'] and $precio > $menor and $precio < $mayor){   array_push($itemList,$item ); 
            }
        }
      }
      ...
      if($filtroCiudad == "" and $filtroTipo != ""){ 
          foreach ($data as $index => $item) {
            $precio = precioNumero($item['Precio']);
            if ($filtroTipo == $item['Tipo'] and $precio > $menor and $precio < $mayor){  array_push($itemList,$item ); 
            }
        }
      }
